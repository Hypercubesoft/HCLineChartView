...
{
    if (hcLinearChartView.xElements == NULL)
    {
        hcLinearChartView.xElements = [NSMutableArray new];
    }
    if (hcLinearChartView.yElements == NULL)
    {
        hcLinearChartView.yElements = [NSMutableArray new];
    }
    [self makeXAndYEqualElementsEqualSize];
    if (hcLinearChartView.sortData)
    {
        [self sortChartData];
    }
}
...
{
    numberOfElements = MIN((int)[hcLinearChartView.xElements count],(int)[hcLinearChartView.yElements count]);
    if (numberOfElements > 0) {
        while ([hcLinearChartView.xElements count] < numberOfElements - 1)
        {
            [hcLinearChartView.xElements removeObjectAtIndex:numberOfElements];
        }
        while ([hcLinearChartView.yElements count] < numberOfElements - 1)
        {
            [hcLinearChartView.yElements removeObjectAtIndex:numberOfElements];
        }
    }
}
...
-(void)sortChartData
{
    [hcLinearChartView.yElements sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        return [[hcLinearChartView.xElements objectAtIndex:[hcLinearChartView.yElements indexOfObject:obj1]] compare:[hcLinearChartView.xElements objectAtIndex:[hcLinearChartView.yElements indexOfObject:obj2]]];
    }];
    [hcLinearChartView.xElements sortUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
...
{
    standardOffset = 10.0;
    chartCornerRadius = hcLinearChartView.chartWithRoundedCorners ? 20.0 : 0.0;
    ...
    chartPointDiameter = MAX(hcLinearChartView.chartLineWidth * 3.0, 8.0);
    axisWidth = 1.0;
    ...
    if (numberOfElements > 0)
    {
        ...
        {
            if ([[hcLinearChartView.yElements objectAtIndex:i] isKindOfClass:[NSNumber class]])
            {
                if ([[hcLinearChartView.xElements objectAtIndex:i] isKindOfClass:[NSNumber class]])
                {
                ...
                else if ([[hcLinearChartView.xElements objectAtIndex:i] isKindOfClass:[NSDate class]])
                {
...
-(void)calculateMinXMaxXMinYMaxY
{
    ...
    for (int i=0 ; i < numberOfElements ; i++)
    {
        double xValue = chartType == chartWithNumericalValues ? [[hcLinearChartView.xElements objectAtIndex:i] doubleValue] : [((NSDate*)[hcLinearChartView.xElements objectAtIndex:i]) timeIntervalSince1970];
        if (xValue < minXValue) {
            minXValue = xValue;
        }
        if ([[hcLinearChartView.yElements objectAtIndex:i] doubleValue]  < minYValue) {
            minYValue = [[hcLinearChartView.yElements objectAtIndex:i] doubleValue];
        }
        if (xValue > maxXValue) {
            maxXValue = xValue;
        }
        if ([[hcLinearChartView.yElements objectAtIndex:i] doubleValue] > maxYValue) {
            maxYValue = [[hcLinearChartView.yElements objectAtIndex:i] doubleValue];
        }
    }
}
...
-(void)calculateMinMaxCoordinatesForYAxis
{
    ...
    {
        double yPosition= [self yPositionForValue:[[hcLinearChartView.yElements objectAtIndex:k] doubleValue]];
...
-(void)findChartValueRangeFoxAxis:(HCAxis)axis
{
    ...
    if (axis == yAxis)
    {
        maxNumberOfValues = chartRect.size.height/(hcLinearChartView.fontSizeForAxis);
    }
    else
    {
        maxNumberOfValues = chartRect.size.width/(hcLinearChartView.fontSizeForAxis);
    }
...
-(void)calculateHorizontalDimensionsForDate
{
    ...
    xAxisLabelTextSize = [self sizeOfText:xAxisString withFontSize:hcLinearChartView.fontSizeForAxis];
    pointsRectTopProportionalDistance = (hcLinearChartView.fontSizeForTitle + (hcLinearChartView.showSubtitle ? hcLinearChartView.fontSizeForSubTitle : 0.0) + 20.0) / chartRect.size.height;
    pointsRectProportionalHeight = (chartRect.size.height * ( 1.0 - pointsRectTopProportionalDistance) - standardOffset * 2.0 - (hcLinearChartView.horizontalValuesOnXAxis ? xAxisLabelTextSize.height : xAxisLabelTextSize.width)) / chartRect.size.height;
}
...
-(void)calculateHorizontalDimensionsForValues
{
    ...
    double maxNumberOfValues = chartRect.size.width/(hcLinearChartView.fontSizeForAxis);
    ...
    NSString* xAxisString1 = hcLinearChartView.showXValueAsCurrency ? [self currencyStringForValue:minXValueOnChart numberOfDecimalPlaces:numberOfXDecimals currencyCode:hcLinearChartView.xAxisCurrencyCode] : [NSString stringWithFormat:decimalFormat,minXValueOnChart];
    NSString* xAxisString2 = hcLinearChartView.showXValueAsCurrency ? [self currencyStringForValue:maxXValueOnChart numberOfDecimalPlaces:numberOfXDecimals currencyCode:hcLinearChartView.xAxisCurrencyCode] : [NSString stringWithFormat:decimalFormat,maxXValueOnChart];
    CGSize textSize1 = [self sizeOfText:xAxisString1 withFontSize:hcLinearChartView.fontSizeForAxis];
    CGSize textSize2 = [self sizeOfText:xAxisString2 withFontSize:hcLinearChartView.fontSizeForAxis];
...
-(void)setupDimensionsForChartWithOneOrZeroPoint
{
    ...
    NSString* yAxisString = [NSString stringWithFormat:@"%f",[[hcLinearChartView.yElements firstObject] doubleValue]];
    xAxisLabelTextSize = [self sizeOfText:xAxisString withFontSize:hcLinearChartView.fontSizeForAxis];
    CGSize yAxisTextSize = [self sizeOfText:yAxisString withFontSize:hcLinearChartView.fontSizeForAxis];
    ...
    pointsRectTopProportionalDistance = (hcLinearChartView.fontSizeForTitle + (hcLinearChartView.showSubtitle ? hcLinearChartView.fontSizeForSubTitle : 0.0) + 20.0) / chartRect.size.height;
    pointsRectProportionalHeight = (chartRect.size.height * ( 1.0 - pointsRectTopProportionalDistance) - 2.0 * standardOffset - (hcLinearChartView.horizontalValuesOnXAxis ? xAxisLabelTextSize.height : xAxisLabelTextSize.width) - MIN(chartRect.size.width, chartRect.size.height) * 2 * leftOrBottomOffsetProportional) / chartRect.size.height;
}
...
-(void)calculateVerticalDimensions
{
    ...
    double maxNumberOfValues = chartRect.size.height/(hcLinearChartView.fontSizeForAxis);
    double potentialTickSize = range/maxNumberOfValues;
    while (potentialTickSize < 1.0)
    {
        potentialTickSize *= 10.0;
    ...
    NSString* yAxisString1 = hcLinearChartView.showYValueAsCurrency ? [self currencyStringForValue:minYValueOnChart numberOfDecimalPlaces:numberOfYDecimals currencyCode:hcLinearChartView.yAxisCurrencyCode] : [NSString stringWithFormat:decimalFormat,minYValueOnChart];
    NSString* yAxisString2 = hcLinearChartView.showYValueAsCurrency ? [self currencyStringForValue:maxYValueOnChart numberOfDecimalPlaces:numberOfYDecimals currencyCode:hcLinearChartView.yAxisCurrencyCode] : [NSString stringWithFormat:decimalFormat,maxYValueOnChart];
    CGSize textSize1 = [self sizeOfText:yAxisString1 withFontSize:hcLinearChartView.fontSizeForAxis];
    CGSize textSize2 = [self sizeOfText:yAxisString2 withFontSize:hcLinearChartView.fontSizeForAxis];
...
-(void)createDots
{
    if (numberOfElements > 1)
    {
        ...
        for (int k=0 ; k < numberOfElements ; k++)
        {
            ...
            if (hcLinearChartView.isValueChartWithRealXAxisDistribution)
            {
                if (chartType == chartWithNumericalValues)
                {
                    xPosition = [self xPositionForValue:[[hcLinearChartView.xElements objectAtIndex:k] doubleValue]];
                }
                else if (chartType == chartWithDateValues)
                {
                    xPosition = [self xPositionForValue:[[hcLinearChartView.xElements objectAtIndex:k] timeIntervalSince1970]];
                }
            }
            else
            {
...
-(void)calculateDotsPositionsForDrawing
{
    if (chartStyle == chartLineWithCircles)
    {
        ...
        while (numberOfPoints < numberOfElements - 1)
        {
            ...
            startPoint.x += (chartPointDiameter - hcLinearChartView.chartLineWidth) /2.0 * xDistance / distance;
            startPoint.y += (chartPointDiameter - hcLinearChartView.chartLineWidth) /2.0 * yDistance / distance;
            endPoint.x -= (chartPointDiameter - hcLinearChartView.chartLineWidth) /2.0 * xDistance / distance;
            endPoint.y -= (chartPointDiameter - hcLinearChartView.chartLineWidth) /2.0 * yDistance / distance;
...
-(void)fixCalculationForDrawing
{
    ...
    if (!hcLinearChartView.isValueChartWithRealXAxisDistribution)
    {
        ...
        currentValueX = chartType == chartWithNumericalValues ? [[hcLinearChartView.xElements firstObject] doubleValue] : [[hcLinearChartView.xElements firstObject] timeIntervalSince1970];
    ...
    else
    {
        if ( chartType == chartWithNumericalValues)
        {
            currentPositionX -= ([((NSNumber*)[hcLinearChartView.xElements firstObject]) doubleValue] - currentValueX) / xAxisTick * xStep;
        }
        else
        {
            currentValueX -= [[NSTimeZone systemTimeZone] secondsFromGMTForDate:[NSDate dateWithTimeIntervalSince1970:currentValueX]];
            currentPositionX -= ([((NSDate*)[hcLinearChartView.xElements firstObject]) timeIntervalSince1970] - currentValueX) / xAxisTick * xStep;
        }
    }
    
    if (chartType == chartWithNumericalValues || (chartType == chartWithDateValues && !hcLinearChartView.isValueChartWithRealXAxisDistribution))
    {
        while (xStep < (hcLinearChartView.horizontalValuesOnXAxis ? xAxisLabelTextSize.width : hcLinearChartView.fontSizeForAxis)) {
            jumpOverX *= 2;
            xStep *= 2.0;
            xAxisTick *= 2.0;
        }
    }
    else if (chartType == chartWithDateValues && hcLinearChartView.isValueChartWithRealXAxisDistribution)
    {
        ...
        for (int i = 0; i < [timeSteps count] - 1; i++)
        {
            ...
            if (singleTimeStep == xAxisDateTick && xStep < (hcLinearChartView.horizontalValuesOnXAxis ? xAxisLabelTextSize.width : hcLinearChartView.fontSizeForAxis))
            {
...
-(void)calculateTicks
{
    int numberOfXValues = hcLinearChartView.horizontalValuesOnXAxis ?
    hcLinearChartView.isValueChartWithRealXAxisDistribution ? fabs(maxXCoordinateOnChart - minXCoordinateOnChart)/(xAxisLabelTextSize.width + 5.0) : MIN(fabs(maxXCoordinateOnChart - minXCoordinateOnChart)/(xAxisLabelTextSize.width + 5.0),numberOfElements) :
    hcLinearChartView.isValueChartWithRealXAxisDistribution ? fabs(maxXCoordinateOnChart - minXCoordinateOnChart + hcLinearChartView.fontSizeForAxis)/(hcLinearChartView.fontSizeForAxis) : MIN(fabs(maxXCoordinateOnChart - minXCoordinateOnChart + hcLinearChartView.fontSizeForAxis)/(hcLinearChartView.fontSizeForAxis),numberOfElements);
    ...
    int numberOfYValues = fabs(maxYCoordinateOnChart - minYCoordinateOnChart + hcLinearChartView.fontSizeForAxis)/(hcLinearChartView.fontSizeForAxis);
    ...
    if (isDate)
    {
        ...
        for (HCTimeStep* singleTimeStep in timeSteps)
        {
            ...
            if (singleTimeStep.alternativeTimeFormat && range > singleTimeStep.referentRangeForAlternativeText)
            {
                ...
                textSize = [self sizeOfText:[self timeStringForDate:[NSDate date] withFormat:singleTimeStep.alternativeTimeFormat] withFontSize:hcLinearChartView.fontSizeForAxis];
            }
            else
            {
                textSize = [self sizeOfText:[self timeStringForDate:[NSDate date] withFormat:singleTimeStep.timeFormat] withFontSize:hcLinearChartView.fontSizeForAxis];
            }
            numberOfValues = chartRect.size.width * pointsRectProportionalWidth / (hcLinearChartView.horizontalValuesOnXAxis ? textSize.width : textSize.height);
